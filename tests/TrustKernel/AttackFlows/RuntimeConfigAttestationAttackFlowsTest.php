...
final class RuntimeConfigAttestationAttackFlowsTest extends TestCase
{
    ...
    public function testPolicyV3StrictRejectsRuntimeConfigSourceSymlinkEvenWhenOnChainMatches(): void
    {
        $fixture = IntegrityFixture::create(['app.txt' => 'ok']);
        ...
        $runtimeConfigRealPath = $fixture->rootDir . DIRECTORY_SEPARATOR . 'config.runtime.json';
        $json = json_encode($runtimeConfig, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
        if (!is_string($json) || @file_put_contents($runtimeConfigRealPath, $json) === false) {
            throw new \RuntimeException('Unable to write runtime config fixture file.');
        }

        $runtimeConfigSymlinkPath = $fixture->rootDir . DIRECTORY_SEPARATOR . 'config.runtime.symlink.json';
        if (@symlink($runtimeConfigRealPath, $runtimeConfigSymlinkPath) === false) {
            $fixture->cleanup();
            self::markTestSkipped('Symlinks are not supported on this platform.');
        }
        ...
        $cfg = new TrustKernelConfig(
            ...
            rpcTimeoutSec: 5,
            runtimeConfigCanonicalSha256: $canonical,
            runtimeConfigSourcePath: $runtimeConfigSymlinkPath,
        );
            ...
            if ($method === 'eth_call') {
                ...
                if ($to === strtolower($instanceController) && $data === $attestationCall) {
                    return json_encode([
                        'jsonrpc' => '2.0',
                        'id' => 1,
                        'result' => Bytes32::normalizeHex($canonical),
                    ], JSON_THROW_ON_ERROR);
                }
        ...
        try {
            $status = $kernel->check();
            self::assertFalse($status->trustedNow);
            self::assertTrue(in_array('runtime_config_source_symlink', $status->errorCodes, true));
        } finally {
            @unlink($runtimeConfigSymlinkPath);
            @unlink($runtimeConfigRealPath);
            $fixture->cleanup();
        }
    }
}
