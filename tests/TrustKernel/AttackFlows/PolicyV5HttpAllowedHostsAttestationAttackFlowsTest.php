...
final class PolicyV5HttpAllowedHostsAttestationAttackFlowsTest extends TestCase
{
    public function testPolicyV5StrictRejectsMismatchedHttpAllowedHostsAttestation(): void
    {
        ...
        $runtimeConfig = [
            'http' => [
                'allowed_hosts' => ['demo.local', 'localhost'],
            ],
            'trust' => [
                'integrity' => [
        ...
        $canonicalConfig = CanonicalJson::sha256Bytes32($runtimeConfig);
        ...
        $expectedAllowedHosts = CanonicalJson::sha256Bytes32([
            'schema_version' => 1,
            'type' => 'blackcat.http.allowed_hosts',
            'hosts' => ['demo.local', 'localhost'],
        ]);
        ...
        $cfg = new TrustKernelConfig(
            ...
            rpcTimeoutSec: 5,
            runtimeConfigCanonicalSha256: $canonicalConfig,
            httpAllowedHostsCanonicalSha256: $expectedAllowedHosts,
        ...
        $hostsKey = strtolower($cfg->httpAllowedHostsAttestationKeyV1);
        $hostsAttestationCall = '0x940992a3' . substr($hostsKey, 2);
        $hostsLockedCall = '0xa93a4e86' . substr($hostsKey, 2);
        ...
        /** @var array<string,mixed> $composerDecoded */
        $composerDecoded = json_decode($composerLock, true, 512, JSON_THROW_ON_ERROR);
        $computedComposer = CanonicalJson::sha256Bytes32($composerDecoded);
        ...
        $transport = new ScenarioTransport(static function (string $url, array $req, int $timeoutSec, int $callIndex) use (
            ...
            unset($url, $timeoutSec, $callIndex);
            ...
            if ($method === 'eth_call') {
                ...
                if ($to === strtolower($instanceController) && $data === '0x9711715a') {
                    $snapshotHex = Abi::snapshotResult(
                        ...
                        activePolicyHash: $cfg->policyHashV5Strict,
                ...
                if ($to === strtolower($instanceController) && $data === $runtimeAttestationCall) {
                    return json_encode(['jsonrpc' => '2.0', 'id' => 1, 'result' => $canonicalConfig], JSON_THROW_ON_ERROR);
                }
                ...
                if ($to === strtolower($instanceController) && $data === $hostsAttestationCall) {
                    // mismatched http.allowed_hosts value
                    return json_encode(['jsonrpc' => '2.0', 'id' => 1, 'result' => '0x' . str_repeat('11', 32)], JSON_THROW_ON_ERROR);
                }
                if ($to === strtolower($instanceController) && $data === $hostsLockedCall) {
                    return json_encode(['jsonrpc' => '2.0', 'id' => 1, 'result' => Abi::boolResult(true)], JSON_THROW_ON_ERROR);
                }

                if ($to === strtolower($instanceController) && $data === $composerAttestationCall) {
                    return json_encode(['jsonrpc' => '2.0', 'id' => 1, 'result' => $computedComposer], JSON_THROW_ON_ERROR);
                }
        ...
        try {
            ...
            self::assertTrue(in_array('http_allowed_hosts_commitment_mismatch', $status->errorCodes, true));
        } finally {
            $fixture->cleanup();
